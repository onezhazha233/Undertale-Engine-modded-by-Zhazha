if(_enabled){
	...
	if(variable_global_exists("prompt_surface")&&surface_exists(global.prompt_surface)&&instance_exists(ui_prompt)){
		draw_surface_ext(global.prompt_surface,base_x+160*SF,base_y+30*SF,SF,SF,0,c_white,1);
	}
}else{
	var SW=(window_get_fullscreen() ? display_get_width() : window_get_width());
	var SH=(window_get_fullscreen() ? display_get_height() : window_get_height());
	...
	gpu_set_blendmode(bm_normal);
	if(variable_global_exists("prompt_surface")&&surface_exists(global.prompt_surface)&&instance_exists(ui_prompt)){
		draw_surface_ext(global.prompt_surface,(SW-640*SF)/2,(SH-480*SF)/2,SF,SF,0,c_white,1);
	}
}
